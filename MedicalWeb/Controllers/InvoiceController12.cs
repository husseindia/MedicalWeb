...
{
    public class InvoiceController12 : Controller
    {
        ...
        public JsonResult Invoice(string pharmName,Guid medicalId, int quantiti)
        {
            ...
            try
            {
                if (pharmaci != null)
                {
                    ...
                    {
                        db.InvoiceClasses.Add(new InvoiceClass { id = Guid.NewGuid(), pharmName = pharmName, medicalName = medical.name, quantiti = quantiti });
                        //db.SaveChanges();
